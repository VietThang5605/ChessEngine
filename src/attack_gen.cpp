...
SF::Bitboard attacks_from_king(const S_BOARD* pos, SF::Color side) {
    SF::Square kingSq = static_cast<SF::Square>(pos->kingSquare[side]); // Lấy ô Vua từ S_BOARD
...
SF::Bitboard attacks_from_sliding(SF::Bitboard sliders, SF::Bitboard occupied, const SF::Direction steps[], int numSteps) {
    ...
    while (b) {
        ...
        for (int i = 0; i < numSteps; ++i) {
            ...
            SF::Square current_sq = from;

            while (true) {
                SF::Square next_sq = static_cast<SF::Square>(current_sq + dir);

                if (!SF::is_ok(next_sq) || SF::distance<SF::Square>(next_sq, current_sq) != 1) {
                    break; 
                }

                attacks |= SF::square_bb(next_sq);
                current_sq = next_sq;
                if (occupied & SF::square_bb(current_sq)) {
                    break; 
                }
            }
        }
