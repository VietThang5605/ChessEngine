...
// 			if (IsRook(piece)) { // if is rook
// 				int sq64 = SQ64(sq);

// 				U64 attackMask = GetSliderAttacks(sq64, pos->allPiecesBB[BOTH], TRUE);
// 				attackMask &= ~(pos->allPiecesBB[side]); // //remove friendly piece mask from attackMask
...
// 			if (IsKnight(piece)) {
// 				U64 attackMask = KnightMask[SQ64(sq)] & (~pos->allPiecesBB[side]);
