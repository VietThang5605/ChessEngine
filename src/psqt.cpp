...
constexpr SF::Score Bonus[SF::PIECE_TYPE_NB][SF::RANK_NB][SF::FILE_NB / 2] = {
...
constexpr SF::Score PBonus[SF::RANK_NB][SF::FILE_NB] = { //pawn bonus
...
// Bảng PSQ cuối cùng 
SF::Score psq[SF::PIECE_NB][SF::SQUARE_NB]; // PIECE_NB=16, SQUARE_NB=64

// Hàm khởi tạo bảng PSQ
void init() { 

    for (SF::Piece pc = SF::W_PAWN; pc <= SF::W_KING; ++pc)
    {
        // Sao chép giá trị vật chất từ quân trắng sang quân đen
        Piecevalue[SF::MG][~pc] = Piecevalue[SF::MG][pc];
        Piecevalue[SF::EG][~pc] = Piecevalue[SF::EG][pc];
    
        // Tính điểm vật chất cơ bản
        SF::Score score = SF::make_score(Piecevalue[SF::MG][pc], Piecevalue[SF::EG][pc]);
    
        // Duyệt qua các ô để tính điểm vị trí
        for (SF::Square s = SF::SQ_A1; s <= SF::SQ_H8; ++s)
        {
          SF::File f = SF::file_of(s); // Lấy file thực tế
          SF::Rank r = SF::rank_of(s); // Lấy rank thực tế
          SF::File qf = SF::map_to_queenside(f); // Lấy file đối xứng bên cánh Hậu (A-D)
            
            // Tính điểm PSQ cho quân trắng
            // SỬA ĐỔI Ở ĐÂY: Dùng type_of(pc) để truy cập Bonus
            psq[ pc][ s] = score + (SF::type_of(pc) == SF::PAWN ? PBonus[r][f] // Dùng file thực 'f' cho Pawn
                                                        : Bonus[SF::type_of(pc)][r][qf]); // Dùng type_of(pc) và file đối xứng 'qf'

            // Tính điểm PSQ cho quân đen (bằng cách lật ô và đảo dấu)
            SF::Square flipped_s = SF::flip_vertically(s);
            if (SF::is_ok(flipped_s)) { // Chỉ gán nếu ô lật hợp lệ
