...
#include <vector>    // Sử dụng trong code hằng số

// --- Định nghĩa các hằng số (Lấy từ Stockfish 11 evaluate.cpp) ---
// Bạn nên cân nhắc tách chúng ra file evaluation_constants.h riêng
...
SF::Value EvaluatePosition(const S_Board* pos) {
    ...
    for (SF::Color c : {SF::WHITE, SF::BLACK}) {
        ...
        while(passers) {
            ...
            if (r_rel >= SF::RANK_4) {
                ...
                if(SF::is_ok(blockSq)) {
                    SF::Square ourKingSq   = static_cast<SF::Square>(Sq120ToSq64[pos->KingSq[c]]);
                    SF::Square theirKingSq = static_cast<SF::Square>(Sq120ToSq64[pos->KingSq[them]]);
                    ...
                    if (pos->pieces[SQ120(blockSq)] == NO_PIECE) {
    ...
    // Sử dụng hàm non_pawn_material từ material_analysis.cpp
     ...
     if (npm_w + npm_b >= EvaluationConstants::SpaceThreshold) {
         for (SF::Color c : {SF::WHITE, SF::BLACK}) {
             ...
             int pieceCount = pos->bigPce[c] - pos->pceNum[(c == SF::WHITE) ? wK : bK]; // Trừ Vua ra
    ...
    complexity += (pos->pceNum[wP] + pos->pceNum[bP]) * 3; // Số Tốt tổng
    complexity += pos->bigPce[SF::WHITE] + pos->bigPce[SF::BLACK]; // Số quân lớn
